...
class Window(ReapyObject):
    ...
    def __init__(
        ...
        if int(self.ptr) == 0:
            raise RuntimeError("window hasn't been connected")
        if ptr is None:
    ...
    def coordinates(self) -> Coordinates:
        return Coordinates(*JS.Window_GetRect(self.ptr)[1:])

    @coordinates.setter
    def coordinates(self, value: Coordinates) -> None:
        self.dimentions = Dimentions.from_coordinates(value)

    @property
    def dimentions(self) -> Dimentions:
        return Dimentions.from_coordinates(self.coordinates)

    @dimentions.setter
    def dimentions(self, dimentions: Dimentions) -> None:
        # c = Coordinates.from_dimentions(dimentions)
        JS.Window_SetPosition(self.ptr, *dimentions.t)

    def focus(self) -> None:
        return JS.Window_SetFocus(self.ptr)
    ...
    def parent(self) -> 'Window':
        """Parent of the Window (if there is one).
    ...
    def position(self, pos: Point) -> None:
        JS.Window_SetPosition(self.ptr, *pos.t, *self.size.t)
    ...
    def bring_to_front(self) -> None:
        ret = JS.Window_SetZOrder(self.ptr, 'TOPMOST')
        print(f'bring_to_front of {self.title} result: {ret}')

    def update(self) -> None:
        JS.Window_Update(self.ptr)
